...
class EnvoyAuth {
  ...
  async fetchNewTokenViaPortal() {
    ...
    // Check if login POST succeeded (should redirect with 3xx)
    if (loginRes.status < 300 || loginRes.status >= 400) {
      this.log(`Entrez portal login failed. Server responded with status: ${loginRes.status} instead of a redirect.`);
    ...
    // Consume body to release socket back to connection pool
    try {
      await loginRes.text();
    } catch (err) {
      // ignore
    ...
    if (!formRes.ok) {
      if (formRes.status === 500) {
        this.log('Entrez token form page returned status 500. This is expected for standard System Owner (non-installer) accounts.');
        return null;
      }
