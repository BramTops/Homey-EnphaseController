...
class EnvoyAuth {
  ...
  async fetchNewTokenViaPortal() {
    ...
    // 1. Establish session by posting credentials to /login
    const loginParams = new URLSearchParams();
    ...
    // Capture cookies (session identifiers) from response
    ...
    // 2. Request the Entrez token generation form page to fetch CSRF token and Site mappings
    ...
    // 4. Auto-discover Site Name associated with the Envoy Serial Number
    ...
    // 5. Submit the token generation form
