...
class EnvoyAuth {
  ...
  async fetchNewTokenViaPortal() {
    ...
    let csrfToken = mainPageCsrfMatch ? mainPageCsrfMatch[1] : null;
    if (!csrfToken) {
    ...
    // Consume body immediately to release socket
    let bodyText = '';
    try {
      bodyText = await loginRes.text();
    } catch (err) {
      this.log(`Failed to read login response body: ${err.message}`);
    }

    // Capture updated CSRF token from the login response body if available
    const postCsrfMatch = bodyText.match(/name="_csrf"\s+value="([^"]+)"/i);
    if (postCsrfMatch) {
      csrfToken = postCsrfMatch[1];
      this.log('Successfully retrieved updated CSRF token from login response body.');
    }

    // Check if login POST succeeded (either 3xx redirect or 200 with no credential errors)
    const isRedirect = loginRes.status >= 300 && loginRes.status < 400;
    const isSuccess200 = loginRes.status === 200 && !bodyText.includes('Wrong username or password') && !bodyText.includes('Invalid credentials');

    if (!isRedirect && !isSuccess200) {
      const errorMatch = bodyText.match(/class="[^"]*error[^"]*"[^>]*>([\s\S]*?)<\/(?:p|div)>/i)
        || bodyText.match(/class="[^"]*alert[^"]*"[^>]*>([\s\S]*?)<\/(?:p|div)>/i);
      const errMsg = errorMatch ? errorMatch[1].trim() : 'Unknown error or invalid credentials';
      this.log(`Entrez portal login failed. Server responded with status: ${loginRes.status}. Details: ${errMsg}`);
      return null;
    }
    ...
    // If the GET request fails (e.g. status 500 on standard System Owner accounts),
    // proceed directly with the POST using the initial login CSRF token and the Envoy serial as the Site fallback.
    this.log('Loading Entrez token form page...');
    let siteName = this.envoySerial;
    let formRes;

    try {
      formRes = await fetch('https://entrez.enphaseenergy.com/entrez_tokens', {
        method: 'GET',
        headers: {
          Cookie: cookieHeader,
          'User-Agent': 'Homey-EnphaseController',
      ...
      if (formRes.ok) {
        ...
        this.log(`Entrez token form page returned status ${formRes.status}. Proceeding to token generation POST with fallback Site and CSRF token.`);
      }
    } catch (err) {
      this.log(`Failed to load Entrez token form page: ${err.message}. Proceeding to token generation POST with fallback Site and CSRF token.`);
    }
    ...
    // 6. Extract the JWT from the <textarea id="*JWT*"> element
    const tokenMatch = responseHtml.match(/id="[^"]*JWT[^"]*"[^>]*>([^<]+)<\/textarea>/i);
  ...
  async fetchNewToken() {
    ...
    let apiResult = null;
    try {
      apiResult = await this.fetchNewTokenViaApi();
    } catch (err) {
      this.log(`API token retrieval path failed: ${err.message}`);
    }

    // If API token has Maintainer/Installer privileges, skip portal scraping entirely
    if (apiResult) {
      const decodedApi = this.evaluateTokenRole(apiResult);
      if (decodedApi.isMaintainer) {
        this.log('API token has Maintainer/Installer privileges. Skipping portal scraping path.');
        return apiResult;
      }
    }
    ...
    try {
      portalResult = await this.fetchNewTokenViaPortal();
    } catch (err) {
  ...
  async getSessionCookie(token, forceRefresh = false) {
    ...
    if (!authResponse.ok) {
      throw new Error(`Local auth check failed. Status: ${authResponse.status}. Body: ${bodyText}`);
    }
