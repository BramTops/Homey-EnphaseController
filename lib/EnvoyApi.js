...
class EnvoyApi extends EnvoyAuth {
   ...
   */
  async setDpelSettings({ enable, export_limit = true, limit_value_W }) {
    const token = await this.getToken();
    let cookie = await this.getSessionCookie(token);
    const host = this.getFormattedHost();
    const dataUrl = `https://${host}/ivp/ss/dpel`;
    ...
    const payload = {
      dynamic_pel_settings: {
        ...
        limit_value_W: Number(limit_value_W),
        slew_rate: 900.0,
    ...
    this.log(`Sending POST request to set DPEL settings to: ${JSON.stringify(payload.dynamic_pel_settings)}...`);
    ...
    if (!response.ok) {
      throw new Error(`DPEL settings command failed. Status: ${response.status}`);
    }
