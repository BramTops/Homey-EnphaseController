...
class EnvoyApi extends EnvoyAuth {
  ...
  async getProductionData() {
    ...
    let consumptionWattsNow = 0;
    let consumptionKwhLifetime = 0;
    ...
    let netConsumptionWattsNow = 0;
    let netConsumptionKwhLifetime = 0;

    if (result && Array.isArray(result.consumption)) {
      const totalCons = result.consumption.find((c) => c.type === 'total-consumption');
      const netCons = result.consumption.find((c) => c.type === 'net-consumption');

      if (totalCons && typeof totalCons.wNow !== 'undefined' && typeof totalCons.whLifetime !== 'undefined' && totalCons.whLifetime > 0) {
        consumptionWattsNow = Math.max(0, totalCons.wNow);
        consumptionKwhLifetime = totalCons.whLifetime / 1000.0;
        hasConsumption = true;
      }
      if (netCons && typeof netCons.wNow !== 'undefined' && typeof netCons.whLifetime !== 'undefined') {
        netConsumptionWattsNow = netCons.wNow;
        netConsumptionKwhLifetime = netCons.whLifetime / 1000.0;
      }
    }
    ...
    return {
      ...
      isMetered,
      consumptionWattsNow,
      consumptionKwhLifetime,
      hasConsumption,
      netConsumptionWattsNow,
      netConsumptionKwhLifetime,
    };
  }
  ...
  /**
   * Check if Envoy has production metering and consumption metering active.
   * @returns {Promise<{ isMetered: boolean, hasConsumption: boolean }>}
   */
  async checkMeterStatus() {
    ...
    try {
      ...
      hasConsumption = !!prodData.hasConsumption;
      this.log(`checkMeterStatus (production.json): isMetered = ${isMetered}, hasConsumption = ${hasConsumption}`);
    } catch (err) {
    ...
    try {
      ...
      this.log('Fetching meters configuration from local Envoy...');
      let response = await fetch(dataUrl, {
        method: 'GET',
        headers: {
          Accept: 'application/json',
      ...
      if (response.ok) {
        ...
        if (Array.isArray(meters)) {
          const prodMeter = meters.find((m) => m.measurementType === 'production');
          const consMeter = meters.find((m) => m.measurementType === 'consumption');

          if (prodMeter && prodMeter.state === 'enabled') {
            isMetered = true;
          }
          if (consMeter && consMeter.state === 'enabled') {
            hasConsumption = true;
          }
        }
      } else {
        this.log(`/ivp/meters returned status: ${response.status}. Skipping.`);
      }
    } catch (err) {
      this.log('Failed to check meter status via /ivp/meters (endpoint may not exist):', err.message);
    }

    this.log(`checkMeterStatus final: isMetered = ${isMetered}, hasConsumption = ${hasConsumption}`);
    return { isMetered, hasConsumption };
  }
}
