...
class EnvoyApi extends EnvoyAuth {
  ...
  async getProductionData() {
    ...
    // Per-phase telemetry, populated from meter channels below (null on single-phase systems)
    let solarpowerPhases = null;
    let gridpowerPhases = null;
    let homepowerPhases = null;
    ...
    try {
      ...
      if (metersConfig && metersConfig.length > 0) {
        ...
        if (Array.isArray(readings)) {
          ...
          // Per-phase breakdown from the same payload (ADR 0011)
          const prodMeters = metersConfig.filter((m) => PRODUCTION_METER_TYPES.includes(m.measurementType) && m.state === 'enabled');
          const prodReading = readings.find((r) => prodMeters.some((m) => m.eid === r.eid));

          solarpowerPhases = EnvoyApi.extractPhases(prodReading);
          gridpowerPhases = EnvoyApi.extractPhases(gridReading);
          homepowerPhases = EnvoyApi.extractPhases(homeReading);

          // Most 3-phase systems have no total-consumption CT: the Envoy derives home load
          // as production + net-consumption. Do the same per phase so the Enphase Home device
          // can show phase load without an extra meter.
          if (!homepowerPhases
            && solarpowerPhases
            && gridpowerPhases
            && solarpowerPhases.length === gridpowerPhases.length) {
            homepowerPhases = solarpowerPhases.map((phase, idx) => {
              const solarWatts = typeof phase.activePower === 'number' ? phase.activePower : null;
              const gridWatts = typeof gridpowerPhases[idx].activePower === 'number' ? gridpowerPhases[idx].activePower : null;
              return {
                activePower: (solarWatts === null || gridWatts === null) ? null : solarWatts + gridWatts,
                voltage: phase.voltage !== null ? phase.voltage : gridpowerPhases[idx].voltage,
                derived: true,
              };
            });
          }
    ...
    return {
      ...
      // Per-phase metrics (null on single-phase systems or when meters are unavailable)
      solarpowerPhases,
      gridpowerPhases,
      homepowerPhases,
      phaseCount: (solarpowerPhases || gridpowerPhases || []).length || 1,
    };
  }

  /**
   * Extract per-phase active power and voltage from a meter reading.
   * Uses the channels[] array that /ivp/meters/readings already returns, so this
   * costs no additional gateway request (see ADR 0011).
   * @param {Object} reading - Single element from /ivp/meters/readings
   * @returns {Array<Object>|null} One entry per phase, or null on single-phase/missing data
   */
  static extractPhases(reading) {
    if (!reading || !Array.isArray(reading.channels) || reading.channels.length < 2) {
      return null;
    }
    return reading.channels.map((channel) => ({
      activePower: typeof channel.activePower === 'number' ? channel.activePower : null,
      voltage: typeof channel.voltage === 'number' ? channel.voltage : null,
    }));
  }
  ...
  async getDpelSettings() {
    ...
    let cookie = await this.getSessionCookie(token);
    const host = this.getFormattedHost();
    const dataUrl = `https://${host}/ivp/ss/dpel`;

    this.log('Fetching DPEL settings from local Envoy...');
    let response = await fetch(dataUrl, {
      method: 'GET',
      headers: {
        Accept: 'application/json',
    ...
    // Handle transient local session eviction by force-refreshing cookie and retrying once (see ADR 0001)
    if (response.status === 401) {
      this.log('Local Envoy returned 401. Retrying with a fresh session cookie...');
      try {
        await response.text();
    ...
    if (!response.ok) {
      throw new Error(`DPEL settings fetch failed. Status: ${response.status}`);
    }
  ...
  async setDpelSettings({
    ...
    // Handle session cookie invalidation by refreshing the local session once
    if (response.status === 401) {
      this.log('Local Envoy returned 401. Retrying with a fresh session cookie...');
      try {
        await response.text();
